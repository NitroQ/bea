...
type Props = {
  ...
  generatingMinutes: Record<string, true>;
};
...
export default function Library({ meetings, notice, onDismissNotice, selectedId, onSelect, onOpen, onCreate, onImport, onImportVtt, onRename, onDelete, onSettings, runtimeReady, selectedEngineName, generatingMinutes }: Props) {
  ...
  return <main className="product-shell">
    ...
    {notice && <div className={`workspace-toast ${/failed|could not|unable|error|needs attention/i.test(notice) ? 'error' : ''}`} role="status" style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 50 }}><Icon name={/failed|could not|unable|error|needs attention/i.test(notice) ? 'x' : 'check'} size={14} /><span>{notice}</span><button type="button" className="workspace-toast-close" onClick={onDismissNotice} aria-label="Dismiss notification"><Icon name="x" size={12} /></button></div>}
  </main>;
}
