...
import { CODEX_FALLBACK_MODELS, LOCAL_PRESETS, PROVIDER_KIND_OPTIONS, providerLabel as kindLabel, withProviderKind } from './providerPresets';
...
/// After a successful login `onVerify` re-runs the provider test so the
/// provider is persisted and the setup gate (`provider.verified`) unlocks.
function CodexSignIn({ provider, onProviderChange, onVerify, verified, message }: { provider: ProviderConfig; onProviderChange: (provider: ProviderConfig) => void; onVerify: () => Promise<void>; verified: boolean; message?: string }) {
  ...
  return <div className="oauth-block">
    ...
    <button className="verify-button" type="button" disabled={signingIn} onClick={() => { setSigningIn(true); setSignInError(null); invoke('codex_oauth_login_command').then(() => onVerify()).catch((error) => setSignInError(`Sign-in failed: ${String(error)}`)).finally(() => setSigningIn(false)); }}>
...
export default function SetupFlow({ status, provider, apiKey, step, onStep, onSelectEngine, onInstallEngine, onImportEngine, onRepairTools, onProviderChange, onApiKeyChange, onTestProvider, onComplete, notice, engineProgress }: Props) {
  ...
  return <main className="setup-shell">
    ...
    <section className="setup-main">
      ...
      {step === 0 && <div className="setup-page"><div className="setup-heading-row"><div className="setup-heading"><span className="page-kicker">01 · Essentials</span><h1>Let’s get your workspace ready.</h1><p>Bea runs quietly on your computer. We’ll check the small pieces it needs before you create your first meeting.</p></div><BeaAvatar variant="waving" size={96} className="setup-mascot" /></div><div className="setup-section"><div className="setup-section-heading"><div><h2>Local tools</h2><p>Required for media conversion and slide OCR.</p></div><span className="section-count">{status.tools.filter((tool) => tool.status === 'ready').length} / {status.tools.length} ready</span></div>{status.tools.map((tool) => <ToolRow tool={tool} key={tool.id} onRepair={() => onRepairTools([tool.id])} />)}</div><div className="setup-note"><Icon name="spark" size={15} /><span>Nothing leaves your computer during setup. Provider keys are only used for a connection test and stored outside the project database.</span></div></div>}
      {step === 1 && <div className="setup-page"><div className="setup-heading-row"><div className="setup-heading"><span className="page-kicker">02 · Transcription</span><h1>Choose how Bea listens.</h1><p>Pick one local engine for this computer. You can add another later in Settings without changing existing meetings.</p></div><BeaAvatar variant="curious" size={96} className="setup-mascot" /></div>{importError && <p className="provider-message error">{importError}</p>}<div className="engine-grid">{status.engines.map((engine) => <EngineCard key={engine.id} engine={engine} selected={engine.id === status.selectedEngine} progress={engineProgress?.id === engine.id ? engineProgress : undefined} onSelect={() => onSelectEngine(engine.id)} onInstall={() => void onInstallEngine(engine.id)} />)}</div><button className="import-model" type="button" onClick={() => void choosePackage()} disabled={importing}><span><Icon name="upload" size={16} /></span><div><strong>{importing ? 'Checking package…' : 'Add from file'}</strong><small>Download failed or offline? Use a verified Bea manifest or supported sherpa-onnx package.</small></div><Icon name="arrow-right" size={15} /></button><button className="import-model secondary-import" type="button" onClick={() => void chooseFolder()} disabled={importing}><span><Icon name="folder" size={16} /></span><div><strong>Add extracted folder</strong><small>Choose a folder with the exact verified engine layout.</small></div><Icon name="arrow-right" size={15} /></button><div className="install-detail"><div><span className="detail-label">Selected engine</span><strong>{selectedEngine?.name ?? 'Choose an engine'}</strong></div><div><span className="detail-label">Download size</span><strong>{selectedEngineSize}</strong></div><div><span className="detail-label">Language coverage</span><strong>{selectedEngine?.languages ?? '—'}</strong></div></div></div>}
      {step === 2 && <div className="setup-page"><div className="setup-heading-row"><div className="setup-heading"><span className="page-kicker">03 · AI provider</span><h1>Connect the thinking layer.</h1><p>Use an OpenAI-compatible endpoint for summaries and minutes. Bea sends packed transcript context only after you confirm the preview.</p></div><BeaAvatar variant="presenting" size={96} className="setup-mascot" /></div><div className="provider-layout"><div className="provider-form"><label>Provider<Select value={provider.kind} options={PROVIDER_KIND_OPTIONS} onChange={(kind) => onProviderChange(withProviderKind(provider, kind))} ariaLabel="Provider" /></label>{provider.kind === 'Local' && <div className="local-presets">{LOCAL_PRESETS.map((preset) => <button key={preset.id} type="button" className={`preset ${provider.base_url === preset.base_url ? 'selected' : ''}`} onClick={() => onProviderChange({ ...provider, base_url: preset.base_url })}>{preset.label}<small>{preset.base_url}</small></button>)}</div>}{provider.kind === 'OpenAiOAuth' && <CodexSignIn provider={provider} onProviderChange={onProviderChange} onVerify={onTestProvider} verified={status.provider.verified} message={status.provider.message} />}{provider.kind !== 'OpenAiOAuth' && <label>Minutes model
<ModelSelect value={provider.model} onChange={(model) => onProviderChange({ ...provider, model })} models={provider.kind === 'OpenRouter' ? openRouterModels : []} ids={provider.kind === 'OpenRouter' ? openRouterModels.map((model) => model.id) : discoveredModels} ariaLabel="Minutes model" />
