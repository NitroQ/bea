...
function CodexSignIn({ provider, onProviderChange, verified, message }: { provider: ProviderConfig; onProviderChange: (provider: ProviderConfig) => void; verified: boolean; message?: string }) {
  ...
  return <div className="oauth-block">
    ...
    <button className="verify-button" type="button" disabled={signingIn} onClick={() => { setSigningIn(true); invoke('codex_oauth_login_command').then(() => onProviderChange({ ...provider, enabled: true })).catch((error) => alert(`Sign-in failed: ${String(error)}`)).finally(() => setSigningIn(false)); }}>
    ...
    <small className="field-help">Uses your ChatGPT/Codex subscription — no API key. Tokens are stored in Windows Credential Manager; a browser tab opens to finish sign-in.</small>
    {message && <p className={`provider-message ${verified ? 'success' : 'error'}`}>{message}</p>}
  </div>;
}
...
export default function SetupFlow({ status, provider, apiKey, step, onStep, onSelectEngine, onInstallEngine, onImportEngine, onRepairTools, onProviderChange, onApiKeyChange, onTestProvider, onComplete, notice, engineProgress }: Props) {
  ...
  useEffect(() => {
    ...
    invoke<string[]>('discover_provider_models_command', { provider: { ...provider, enabled: true }, apiKey })
      .then(setDiscoveredModels)
      .catch(() => setDiscoveredModels([]));
  }, [provider.kind, provider.base_url, apiKey]);
