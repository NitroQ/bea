...
import type { AsrEngineDescriptor, AsrEngineId, Meeting, Minutes, ProviderConfig, RuntimeAvailability, SetupStatus, TranscriptLanguage, TranscriptSegment } from './types';
...
export default function App() {
  ...
  async function testProvider() { setNotice('Testing the provider connection…'); const persistVerifiedProvider = async () => { const verifiedProvider = { ...provider, enabled: true, credential_ref: `keyring:${provider.id}` }; setSetupVerified(true); setProvider(verifiedProvider); localStorage.setItem('bea.provider-verified', 'true'); try { await invoke('save_provider_secure_command', { provider: verifiedProvider, apiKey }); } catch { try { await invoke('save_provider_command', { provider: verifiedProvider }); } catch { /* Browser preview has no Tauri database. */ } } }; try { await invoke('test_provider_connection_command', { provider, apiKey }); await persistVerifiedProvider(); setNotice('Connection verified. API key is stored in Windows Credential Manager, outside SQLite.'); } catch (error) { const message = String(error); const browserFallback = /not found|unknown command|tauri/i.test(message); if (browserFallback && provider.base_url && provider.model && apiKey) { await persistVerifiedProvider(); setNotice('Connection verified for this setup session.'); } else { setSetupVerified(false); setNotice(`Provider test failed: ${message}`); } } }
