...
export default function App() {
  ...
  if (route.screen === 'settings') return <SettingsScreen status={setupStatus} provider={provider} apiKey={apiKey} onBack={() => { window.location.hash = '#library'; }} onRefresh={refreshRuntime} onRepair={repairTools} onProviderChange={setProvider} onApiKeyChange={setApiKey} onTest={testProvider} onSelectEngine={selectEngine} onInstallEngine={installEngine} onReset={() => { localStorage.removeItem('bea.setup-complete'); setSetupComplete(false); setSetupStep(0); }} />;
...
function SettingsScreen({ status, provider, apiKey, onBack, onRefresh, onRepair, onProviderChange, onApiKeyChange, onTest, onSelectEngine, onInstallEngine, onReset }: { status: SetupStatus; provider: ProviderConfig; apiKey: string; onBack: () => void; onRefresh: () => void; onRepair: (ids: Array<'ffmpeg' | 'tesseract'>) => Promise<void>; onProviderChange: (provider: ProviderConfig) => void; onApiKeyChange: (value: string) => void; onTest: () => Promise<void>; onSelectEngine: (id: AsrEngineId) => void; onInstallEngine: (id: AsrEngineId) => Promise<void>; onReset: () => void }) { const keyRequired = provider.kind !== 'Local' && provider.kind !== 'OpenAiOAuth'; const [openRouterModels, setOpenRouterModels] = useState<OpenRouterModelInfo[]>([]); useEffect(() => { invoke<OpenRouterModelInfo[]>('fetch_openrouter_models_command').then(setOpenRouterModels).catch(() => setOpenRouterModels([])); }, []); const [discoveredIds, setDiscoveredIds] = useState<string[]>([]); useEffect(() => { if (provider.kind !== 'Local' && provider.kind !== 'OpenAiCompatible') return; if (!provider.base_url || !apiKey) { setDiscoveredIds([]); return; } invoke<string[]>('discover_provider_models_command', { provider, apiKey }).then(setDiscoveredIds).catch(() => setDiscoveredIds([])); }, [provider.kind, provider.base_url, provider.id, apiKey]); return <main className="settings-shell"><header className="settings-header"><button className="back-link" onClick={onBack}><Icon name="arrow-left" size={15} />Meetings</button><div><span className="page-kicker">Workspace settings</span><h1>Keep Bea ready.</h1><p>Repair local tools, switch transcription engines, or rotate the provider connection.</p></div><button className="secondary" onClick={onRefresh}><Icon name="refresh" size={14} />Refresh checks</button></header><div className="settings-grid"><section className="settings-section"><div className="settings-section-title"><div><h2>Local tools</h2><p>Required for media conversion and OCR.</p></div></div>{status.tools.map((tool) => <div className="settings-row" key={tool.id}><span className={`setup-status ${tool.status === 'ready' ? 'is-ready' : ''}`}><Icon name={tool.status === 'ready' ? 'check' : 'download'} size={14} /></span><span><strong>{tool.name}</strong><small>{tool.detail}</small></span><button className="secondary small" onClick={() => void onRepair([tool.id])}>{tool.status === 'ready' ? 'Repair' : 'Install'}</button></div>)}</section><section className="settings-section"><div className="settings-section-title"><div><h2>Transcription</h2><p>Existing meetings keep the engine they were created with.</p></div></div>{status.engines.map((engine) => <button type="button" key={engine.id} className={`settings-engine ${status.selectedEngine === engine.id ? 'selected' : ''}`} onClick={() => engine.status === 'ready' ? onSelectEngine(engine.id) : void onInstallEngine(engine.id)}><span><Icon name="waveform" size={15} /><strong>{engine.name}</strong><small>{engine.languages} · {engine.size}</small></span><span className={`state-label ${engine.status === 'ready' ? 'ready' : 'attention'}`}>{engine.status === 'ready' ? 'Installed · Select' : 'Install package'}</span></button>)}</section><section className="settings-section provider-settings"><div className="settings-section-title"><div><h2>AI provider</h2><p>Keys are held outside your project database.</p></div><span className={`state-label ${status.provider.verified ? 'ready' : 'attention'}`}>{status.provider.verified ? 'Verified' : 'Needs test'}</span></div><label>Provider<select value={provider.kind} onChange={(event) => onProviderChange({ ...provider, kind: event.target.value as ProviderConfig['kind'], base_url: DEFAULT_BASE_URLS[event.target.value as ProviderConfig['kind']] ?? '' })}><option value="OpenRouter">OpenRouter</option><option value="OpenAiCompatible">OpenAI-compatible</option><option value="ClaudeCompatible">Claude-compatible proxy</option><option value="Local">Local server (LM Studio / Ollama / llama.cpp)</option><option value="OpenAiOAuth">OpenAI (ChatGPT sign-in)</option></select></label>{provider.kind === 'Local' && <div className="local-presets">{LOCAL_PRESETS.map((preset) => <button key={preset.id} type="button" className={`preset ${provider.base_url === preset.base_url ? 'selected' : ''}`} onClick={() => onProviderChange({ ...provider, base_url: preset.base_url })}>{preset.label}<small>{preset.base_url}</small></button>)}</div>}{provider.kind === 'OpenAiOAuth' && <CodexSignInSettings provider={provider} onProviderChange={onProviderChange} verified={status.provider.verified} />}{provider.kind !== 'OpenAiOAuth' && <label>Model<input list="settings-models" value={provider.model} onChange={(event) => onProviderChange({ ...provider, model: event.target.value })} placeholder={provider.kind === 'OpenRouter' ? 'Pick from the OpenRouter catalog' : 'Model id'} /><datalist id="settings-models">{(provider.kind === 'OpenRouter' ? openRouterModels.map((model) => model.id) : discoveredIds).map((id) => <option key={id} value={id} />)}</datalist><small className="field-help">Default model for minutes and chat. A meeting can override it in its Chat tab.</small></label>}{provider.kind !== 'OpenAiOAuth' && <label>Base URL<input value={provider.base_url} onChange={(event) => onProviderChange({ ...provider, base_url: event.target.value })} /></label>}{provider.kind !== 'Local' && provider.kind !== 'OpenAiOAuth' && <label>API key<input type="password" value={apiKey} onChange={(event) => onApiKeyChange(event.target.value)} placeholder="Enter to rotate key" autoComplete="off" /></label>}{provider.kind !== 'OpenAiOAuth' && <button className="primary" onClick={() => void onTest()} disabled={!provider.base_url || !provider.model || (keyRequired && !apiKey)}><Icon name="spark" size={14} />Test and save provider</button>}</section></div><div className="settings-danger"><div><strong>Start setup again</strong><p>Use this only if you want to change the required first-run choices.</p></div><button className="secondary" onClick={onReset}>Reset setup gate</button></div></main> }
