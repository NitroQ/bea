...
export default function MeetingWorkspace({ meeting, onBack, onNotice, onImport, onImportVtt, onRecording, onMeetingStatus, onRetryTranscription, onRepairTools, repairing = [], busy, transcribeProgress, liveSegments = [] }: Props) {
...
function Overview({ transcript, minutes, meeting, onOpenTranscript, onGenerate, generating, onRetry, onRepairTools, toolsInstalling }: { transcript: TranscriptSegment[]; minutes: Minutes; meeting: Meeting; onOpenTranscript: () => void; onGenerate: () => void; generating: boolean; onRetry: () => void; onRepairTools: () => void; toolsInstalling: boolean }) {
  // "Needs attention" must explain itself: surface the persisted failure
  // reason plus the two actions that usually fix it — retrying, and (when the
  // error smells like missing media tooling) installing FFmpeg/FFprobe.
