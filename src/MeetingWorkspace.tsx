...
import { parseSlashCommand, parseVisionSuffix } from './chatActions';
...
export default function MeetingWorkspace({ meeting, onBack, onNotice, onImport, onImportVtt, onRecording, onMeetingStatus, onRetryTranscription, busy, transcribeProgress, liveSegments = [] }: Props) {
  ...
  // Optional "include video frames" input for minutes generation: comma-separated seconds.
  ...
  async function sendChat(input: string) {
    ...
    setChatBusy(true);
    const { question, includeFrames } = parseVisionSuffix(input);
    try {
  ...
  function parseFrameInput(value: string): number[] | undefined {
    const list = value.split(',').map((part) => Number.parseInt(part.trim(), 10)).filter((n) => Number.isFinite(n) && n >= 0);
  ...
  async function runMinutesGeneration() {
    setGenerating(true);
    try {
      const generated = await invoke<Minutes>('generate_minutes_command', { meetingId: meeting.id, includeFrames: parseFrameInput(minutesFrameInput) });
