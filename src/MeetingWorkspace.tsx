...
type Props = { meeting: Meeting; onBack: () => void; onNotice: (message: string) => void; onImport: (meeting: Meeting) => void; onImportVtt: (meeting: Meeting) => void; onRecording: (meeting: Meeting, action: 'start' | 'pause' | 'resume' | 'stop', deviceIds?: string[]) => void; onMeetingStatus: (meetingId: string, status: Meeting['status']) => void; onRetryTranscription: (meeting: Meeting) => void; onResumeTranscription: (meeting: Meeting) => void; onRepairTools?: (ids: Array<'ffmpeg' | 'tesseract'>) => Promise<void>; repairing?: string[]; busy?: boolean; transcribeProgress?: { completed: number; total: number } | null; liveSegments?: TranscriptSegment[]; generating: boolean; onGenerateMinutes: (meeting: Meeting) => Promise<void>; onCancelMinutesGeneration: (meetingId: string) => void };
type Tab = 'transcript' | 'minutes' | 'evidence' | 'chat';
...
export default function MeetingWorkspace({ meeting, onBack, onNotice, onImport, onImportVtt, onRecording, onMeetingStatus, onRetryTranscription, onResumeTranscription, onRepairTools, repairing = [], busy, transcribeProgress, liveSegments = [], generating, onGenerateMinutes, onCancelMinutesGeneration }: Props) {
  const [tab, setTab] = useState<Tab>('transcript');
  // Resume availability: failed meetings with completed chunks offer
  // "Resume transcription" instead of a from-scratch retry.
  const [resumeAvailable, setResumeAvailable] = useState(false);
  useEffect(() => {
    if (meeting.status !== 'failed') { setResumeAvailable(false); return; }
    let active = true;
    invoke<{ done: number }>('get_transcription_progress_command', { meetingId: meeting.id })
      .then((progress) => { if (active) setResumeAvailable(progress.done > 0); })
      .catch(() => { if (active) setResumeAvailable(false); });
    return () => { active = false; };
  ...
  useEffect(() => {
    ...
    invoke<ProviderConfig | null>('load_provider_command', { providerId: 'primary' }).then((saved) => {
      ...
      if (kind === 'OpenAiOAuth') {
        invoke<string[]>('codex_list_models_command').then((models) => { if (!cancelled) setChatModelOptions(codexModelOptionsOrFallback(models)); }).catch(() => { if (!cancelled) setChatModelOptions(CODEX_FALLBACK_MODELS); });
  ...
  // Evidence/segment clicks may arrive while the player is on another tab —
  // remember the position and apply it once the transcript tab re-mounts it.
  const pendingSeekRef = useRef<number | null>(null);
  useEffect(() => {
    if (tab !== 'transcript' || pendingSeekRef.current === null) return;
    const seconds = pendingSeekRef.current;
    pendingSeekRef.current = null;
    const media = mediaRef.current;
    if (media) { media.currentTime = seconds; setPlaybackSeconds(seconds); }
  }, [tab, mediaUrl]);
  ...
  // Listen for background minutes generation completing while the workspace is open.
  useEffect(() => {
    const handler = (event: Event) => {
      const detail = (event as CustomEvent<{ meetingId: string; minutes: Minutes }>).detail;
      if (detail?.meetingId === meeting.id) {
        const generic = !detail.minutes.title.trim() || /^(meeting minutes|minutes)$/i.test(detail.minutes.title.trim());
        setMinutes(generic ? { ...detail.minutes, title: meeting.title } : detail.minutes);
      }
    };
    window.addEventListener('bea:minutes-generated', handler as EventListener);
    return () => window.removeEventListener('bea:minutes-generated', handler as EventListener);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (playable?.path) {
        ...
        setIsVideo(mediaKindIsVideo(playable.kind));
  ...
  function startGeneration(): Promise<void> {
    ...
    const task = (async () => {
      try {
        await onGenerateMinutes(meeting);
  ...
  function closeClarifications() {
    ...
    if (generationRef.current) {
      ...
      onCancelMinutesGeneration(meeting.id);
  ...
  const conversationBlocks = useMemo<ConversationBlock[]>(() => buildConversationBlocks(mergedTranscript), [mergedTranscript]);
  ...
  async function exportMinutes(format: 'markdown' | 'pdf' | 'docx') {
    try {
      // Nothing generated yet → say so instead of writing an empty document.
      if (!minutesHasExportableContent(minutes)) { notify('Generate minutes first — there is nothing to export yet.'); return; }
      await save();
      // Browser preview: only markdown can download; file exports need Tauri.
      if (!inDesktop) {
        if (format === 'markdown') { downloadMarkdown(); return; }
        notify('File exports need the desktop app — open Bea in Tauri to export.');
        return;
      }
      // Desktop: every format goes through the backend exporter — WebView2
      // silently swallows anchor downloads, so a blob download would report
      // success while nothing landed on disk.
  ...
  /// for subtitle players.
  function buildTranscriptText(): string {
    return conversationBlocks.filter((block) => !isSilenceSegment({ text: block.text } as TranscriptSegment)).map((block) => {
      const who = speakerLabel(block.speaker, speakerNames);
      const timestamp = mmss(block.startSeconds);
      return `[${timestamp}] ${who}:\n${block.text}`;
    }).join('\n\n');
  }
  function buildTranscriptMarkdown(): string {
    const lines = conversationBlocks.filter((block) => !isSilenceSegment({ text: block.text } as TranscriptSegment)).map((block) => {
      const who = speakerLabel(block.speaker, speakerNames);
      const timestamp = mmss(block.startSeconds);
      return `**[${timestamp}] ${who}**\n\n${block.text}`;
    });
    return `# Transcript \u2014 ${meeting.title}\n\n${lines.join('\n\n')}`;
  }
  function buildTranscriptVtt(): string {
    const cues = conversationBlocks.filter((block) => !isSilenceSegment({ text: block.text } as TranscriptSegment)).map((block) => {
      const start = formatVttTime(block.startSeconds);
      const end = formatVttTime(block.endSeconds);
      const who = speakerLabel(block.speaker, speakerNames);
      return `${start} --> ${end}\n${who}: ${block.text}`;
    });
    return `WEBVTT\n\n${cues.join('\n\n')}`;
  ...
  async function downloadTranscript(format: 'txt' | 'markdown' | 'vtt') {
    if (!conversationBlocks.length) { notify('There is no transcript to export yet.'); return; }
    const text = format === 'vtt' ? buildTranscriptVtt() : format === 'markdown' ? buildTranscriptMarkdown() : buildTranscriptText();
    const mime = format === 'vtt' ? 'text/vtt' : format === 'markdown' ? 'text/markdown' : 'text/plain';
    const ext = format === 'markdown' ? 'md' : format;
    const safeTitle = meeting.title.replace(/[^a-z0-9]+/gi, '-').toLowerCase() || 'transcript';
    ...
    if (inDesktop) {
      try {
        ...
        notify(`${ext.toUpperCase()} transcript saved.`);
      } catch (error) { notify(`Unable to export transcript: ${String(error)}`); }
    } else {
      const anchor = document.createElement('a');
      anchor.href = URL.createObjectURL(blob);
      anchor.download = defaultName;
      anchor.click();
      URL.revokeObjectURL(anchor.href);
      notify(`${ext.toUpperCase()} transcript downloaded.`);
    }
  }
  /// Editing a merged conversation block rewrites its first transcript segment
  /// and deletes the later members — otherwise the pre-merge wording of those
  /// members stays in the database and the old text reappears after a reload.
  async function updateSegment(block: ConversationBlock, text: string) {
    setTranscript((items) => applyBlockEdit(items, block, text));
    try {
      await invoke('update_transcript_segment_command', { segmentId: block.key, text });
      const rest = block.segmentIds.slice(1);
      if (rest.length) await invoke('delete_transcript_segments_command', { meetingId: meeting.id, segmentIds: rest });
    } catch (error) {
      notify(`Edit may not be saved: ${String(error)}`);
    }
  }
  /// Speaker-picker apply: selection[0] is the primary speaker, the rest are
  /// overlapping speakers. An empty selection clears the block entirely. Every
  /// member of a merged block gets the same speaker — otherwise the block
  /// splits on the next rebuild and the members revert after a reload.
  async function applySpeakerSelection(block: ConversationBlock, selection: number[]) {
    const primary = selection.length ? selection[0] : null;
    const members = new Set(block.segmentIds);
    setTranscript((items) => items.map((item) => members.has(item.id) ? { ...item, speaker: primary } : item));
    setSegmentSpeakers((current) => { const next = { ...current }; if (selection.length > 1) next[block.key] = selection; else delete next[block.key]; return next; });
    try {
  ...
  /// Evidence links are play buttons: jump to the transcript tab and seek the
  /// player to the quote's start, so the button never acts dead.
  function seekFromEvidence(seconds: number) { pendingSeekRef.current = seconds; setTab('transcript'); }
...
type TranscriptFormatId = 'txt' | 'markdown' | 'vtt';
const TRANSCRIPT_FORMATS: Array<{ id: TranscriptFormatId; label: string; ext: string; hint: string; icon: IconName }> = [
  { id: 'txt', label: 'Plain Text', ext: '.txt', hint: 'Simple text format', icon: 'file' },
  { id: 'markdown', label: 'Markdown', ext: '.md', hint: 'Formatted text', icon: 'file' },
  { id: 'vtt', label: 'WebVTT', ext: '.vtt', hint: 'Subtitle format', icon: 'download' },
];
function TranscriptExportMenu({ onExport }: { onExport: (format: TranscriptFormatId) => void }) {
  const [open, setOpen] = useState(false);
  const menuRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (!open) return;
  ...
  return <div className="export-menu" ref={menuRef}><button type="button" className="secondary small" aria-haspopup="menu" aria-expanded={open} onClick={() => setOpen((value) => !value)}><Icon name="download" size={14} />Export<Icon name="chevron-down" size={13} /></button>{open && <div className="export-menu-list" role="menu" aria-label="Export transcript as">{TRANSCRIPT_FORMATS.map((format) => <button key={format.id} type="button" className="export-menu-item" role="menuitem" onClick={() => { setOpen(false); onExport(format.id); }}><Icon name={format.icon} size={15} /><span><strong>{format.label} <small>{format.ext}</small></strong><small>{format.hint}</small></span></button>)}</div>}</div>;
}
